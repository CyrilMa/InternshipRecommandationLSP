...
class score:
    ...
    def randomExploPLUS2(self,tstart,tend,tmesure,tendtraining,nb_links,trainingtimes,confmetrics,nb_iter,sc1,sc3):
        #Random exploration of the parameter space between the two values given for each metric in the config file (with classes)
        ...
        for i in range(nb_iter): #check all combinaison of parameters
            ...
            if not allnull:
                ...
                ev1.calculateScoreFromTimeAggreg({x:self._ranks[x] for x in sc1._ranks},trainingtimes)
                ev3.calculateScoreFromTimeAggreg({x:self._ranks[x] for x in sc3._ranks},trainingtimes)
                meanF = meanFscore(ev1._F,ev3._F)
        ...
        return initconfmetrics1,initconfmetrics3
    ...
    def gradDescentLinExp(self,tstart,tend,tmesure,tendtraining,nb_links,trainingtimes,init,derstep,sizelinexptep,numlinexptep,GDMaxstep):
        ...
        while maxindex!=0 and k <maxstep: #no change or 100 step
            ...
            maxconfmetrics={metric: 1 for metric in confmetrics}
    ...
    def gradDescentLinExpPLUS(self,tstart,tend,tmesure,tendtraining,nb_links,trainingtimes,init1,init2,init3,derstep,sizelinexptep,numlinexptep,maxstep,sc1,sc2,sc3):
        ...
        sc3.rankPairs(tmesure,tendtraining,init3)
        Mergeranks=sc1._ranks.copy()
        Mergeranks.update(sc2._ranks)
        Mergeranks.update(sc3._ranks)
        self._ranks=Mergeranks
    ...
    def gradDescentLinExpPLUS2(self,tstart,tend,tmesure,tendtraining,nb_links,trainingtimes,init1,init3,derstep,sizelinexptep,numlinexptep,maxstep,sc1,sc2,sc3):
        #Gradient descent witho classes
        ...
        if k== maxstep:
            sys.stderr.write("WARNING: Nb max interation\n")
        return confmetrics1,confmetrics3,maxev,maxev1,maxev3
