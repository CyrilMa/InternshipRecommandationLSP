...
if len(sys.argv)<2:
	...
	conf = open(path_to_conf+sys.argv[1], 'r')
...
sys.stdout.write("tendpred: "+str(tendpred)+"\n")

config_dict = {"tstartobs":tstartobs, "tendobs":tendobs, "tstartpred":tstartpred, "tendpred":tendpred}
print(config_dict)
t = 0
nb_linksPRED = 0
...
classthreshold = 5
for u,v in itertools.product(obsnodes.keys(),classnodes.keys()): #to predict new pair of nodes and initialize the classes
	link = (u,v)
	sc.addPair(link)
	scPLUS.addPair(link)
...
#ALL
metrics.computeMetrics(sc,tstartobsT,tendobsT,obstimes,obsnodes,classnodes) #compute all metrics
...
#C1
metrics.computeMetrics(sc1,tstartobsT,tendobsT,obstimes,obsnodes,classnodes) #compute all metrics
sc1.integrateMetrics(tstartpredT,tendpredT) #integrate metrics
sc1.setMaxByMetric()
sc1.normalizeMetrics()
#C2
metrics.computeMetrics(sc2,tstartobsT,tendobsT,obstimes,obsnodes,classnodes) #compute all metrics
sc2.integrateMetrics(tstartpredT,tendpredT) #integrate metrics
sc2.setMaxByMetric()
sc2.normalizeMetrics()
#C3
metrics.computeMetrics(sc3,tstartobsT,tendobsT,obstimes,obsnodes,classnodes) #compute all metrics
sc3.integrateMetrics(tstartpredT,tendpredT) #integrate metrics
sc3.setMaxByMetric()
sc3.normalizeMetrics()
#PLUS
metrics.computeMetrics(scPLUS,tstartobsT,tendobsT,obstimes,obsnodes,classnodes) #compute all metrics
...
metrics.computeMetrics(sc,tstartobs,tendobs,obstimes,obsnodes,classnodes)
sc.integrateMetrics(tstartpred,tendpred)
sc.setMaxByMetric()
sc.normalizeMetrics()
sc.rankPairs(tstartpred,tendpred,predconfmetric)

metrics.computeMetrics(sc1,tstartobs,tendobs,obstimes,obsnodes,classnodes)
sc1.integrateMetrics(tstartpred,tendpred)
sc1.setMaxByMetric()
sc1.normalizeMetrics()
sc1.rankPairs(tstartpred,tendpred,predconfmetric1)

metrics.computeMetrics(sc2,tstartobs,tendobs,obstimes,obsnodes,classnodes)
sc2.integrateMetrics(tstartpred,tendpred)
sc2.setMaxByMetric()
sc2.normalizeMetrics()
sc2.rankPairs(tstartpred,tendpred,predconfmetric2)

metrics.computeMetrics(sc3,tstartobs,tendobs,obstimes,obsnodes,classnodes)
...
nb_linksPredictedC1,nb_linksPredictedC2,nb_linksPredictedC3=0,0,0
predicted_links = open(path_to_exports+sys.argv[2]+"_pred","w")

for link in sc._ranks:
	predicted_links.write(str(link[0])+" "+str(link[1])+" "+str(scPLUS._ranks[link])+"\n")
	if link in sc1._pair_set:
		nb_linksPredictedC1 += scPLUS._ranks[link]
	elif link in sc2._pair_set:
		nb_linksPredictedC2 += scPLUS._ranks[link]
...
from evaluate_model import fast_scoring
score = fast_scoring(path_to_exports+sys.argv[2]+"_pred","graph/"+sys.argv[2]+".txt",config_dict)
wfile = open(path_to_exports+"msse", "a")
wfile.write(sys.argv[2] + " " + sys.argv[1] + " " + str(score) + "\n")
wfile.close()

output = sys.argv[2] + " " + sys.argv[1] + " " + str(ev._Pr) + " " + str(ev._Rc) + " " + str(ev._F) + " " + str(ev01._Pr) + " " + str(ev01._Rc) + " " + str(ev01._F) + " " +str(ev02._Pr) + " " + str(ev02._Rc) + " " + str(ev02._F) + " " +str(ev03._Pr) + " " + str(ev03._Rc) + " " + str(ev03._F) + " " + str(evPLUS._Pr) + " " + str(evPLUS._Rc) + " " + str(evPLUS._F) + " " +str(ev1._Pr) + " " + str(ev1._Rc) + " " + str(ev1._F) + " " + str(ev2._Pr) + " " + str(ev2._Rc) + " " + str(ev2._F) + " " + str(ev3._Pr) + " " + str(ev3._Rc) + " " + str(ev3._F) + "\n"
wfile = open(path_to_exports+"prf", "a")
wfile.write(output)
wfile.close()
