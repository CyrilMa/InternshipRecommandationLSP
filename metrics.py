...
class metrics:
    ...
    def computeMetrics(self,sc,tstart,tend,times,nodes,classnodes):
        ...
        self.cluster_mat = np.load("graph/clusters_sim.npy")
        ...
        for metricsname in self._confmetrics: #launch each metrics in confmetrics
            ...
            if metricsname == "PairActivityExtrapolation":
                ...
                for link in sc._pair:
                    u,v=link
                    sc.addFunction(link,"commonNeighbors",self.commonNeighbors(nodes[u],classnodes[v]))
                ...
                for link in sc._pair:
                    u,v=link
                    sc.addFunction(link,"jaccardIndex",self.jaccardIndex(nodes[u],classnodes[v],nodes,classnodes))
            ...
            elif metricsname == "clusterSimilarity":
                for link in sc._pair:
                    u,v=link
                    sc.addFunction(link,"clusterSimilarity",self.clusterSimilarity(nodes[u],v))
    ...
    def commonNeighbors(self,nodes_u,nodes_v):
        interactions = set()
        for w in nodes_u:
            interactions |= set(self.classnodes[w] & nodes_v)
        n = len(interactions)
        del(interactions)
        return lambda t: float(n) #modif!

    def jaccardIndex(self,nodes_u,nodes_v,nodes,classnodes):
        interactions_u = set()
        interactions_v = set()

        for w in nodes_u:
            interactions_v |= set(classnodes[w] & nodes_v)
        for w in nodes_v:
            interactions_u |= set(nodes[w] & nodes_u)

        n = len(interactions_u)
        m = len(interactions_v)
        del(interactions_u, interactions_v)
        return lambda t: float(m+n) #modif!
    ...
    def adamicAdar(self,u,v,nodes,classnodes):
        s=0.
        for k in nodes[u].intersection(nodes[v]):
...
#ACTIVITY EXTRAPOLATION:
    ...
    def clusterSimilarity(self,nodes_u,v):
        try:
            sim = min(self.cluster_mat[w,v] for w in nodes_u if w != v)
        except ValueError:
            sim=0
        return lambda t:sim
